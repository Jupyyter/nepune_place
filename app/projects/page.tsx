...
// Note: Static imports for thumbnails are good practice if they are above the fold or critical.
// However, for the project images themselves, string paths are fine as they are loaded dynamically.
// import jhonnyImg from "/public/imgs/jhonny.png"; // Example, not strictly needed if paths are strings
...
function Projects() {
  ...
  // --- EFFECT FOR PROJECT CHANGE: Reset media, destroy players, AND PRELOAD IMAGES ---
  useEffect(() => {
    setCurrentMediaIndex(0); // Reset to first media item

    // Destroy old player instances if they exist
    ...
    if (fullscreenPlayerRef.current?.destroy) {
      try { fullscreenPlayerRef.current.destroy(); } catch(e) { console.error("Error destroying fullscreen player:", e); }
      fullscreenPlayerRef.current = null;
    }

    // **** NEW: PRELOAD IMAGES FOR THE NEWLY SELECTED PROJECT ****
    if (selectedProject && typeof window !== 'undefined') { // Ensure window is defined (for Image object)
      // console.log(`Preloading images for ${selectedProject.title}`); // Optional: for debugging
      selectedProject.images.forEach(imageUrl => {
        if (imageUrl && typeof imageUrl === 'string') { // Basic check for valid URL
          const img = new window.Image(); // Use window.Image to be explicit
          img.src = imageUrl;
          // Optional: Add onload/onerror for debugging preloading
          // img.onload = () => console.log(`Preload success: ${imageUrl}`);
          // img.onerror = () => console.error(`Preload FAILED: ${imageUrl}`);
        }
      });
    }
    // The YouTube player initialization will be handled by the other useEffect
    // that depends on [playerReady, selectedProject, currentMediaIndex, isFullscreen]
 ...
 useEffect(() => { if (typeof window !== 'undefined' && (!(window as any).YT || !(window as any).YT.Player)) { const tag = document.createElement('script'); tag.src = 'https://www.youtube.com/iframe_api'; document.body.appendChild(tag); (window as any).onYouTubeIframeAPIReady = () => { setPlayerReady(true); }; } else if (typeof window !== 'undefined') { setPlayerReady(true); } return () => { if (typeof window !== 'undefined' && typeof (window as any).onYouTubeIframeAPIReady === 'function') { delete (window as any).onYouTubeIframeAPIReady; }}; }, []);

  const createPlayer = (elementId: string, playerRef: React.MutableRefObject<any>, onReadyCallback?: (event: any) => void) => { if (!playerReady || typeof window === 'undefined' || !document.getElementById(elementId)) return; if (playerRef.current?.destroy) { try { playerRef.current.destroy(); } catch (e) { console.error(e); } playerRef.current = null; } playerRef.current = new (window as any).YT.Player(elementId, { events: { 'onReady': (event: any) => { if (onReadyCallback) onReadyCallback(event); }, 'onError': (event: any) => { console.error(`YT Error ${elementId}:`, event.data); }, } }); };
  ...
  useEffect(() => { if (isFullscreen && playerReady && selectedProject && selectedProject.videoUrl && currentMediaIndex === 0) { const fullscreenElementId = `youtube-iframe-fullscreen-${selectedProject.id}`; const checkAndCreate = () => { if (document.getElementById(fullscreenElementId)) { createPlayer(fullscreenElementId, fullscreenPlayerRef, (event) => { event.target.seekTo(videoCurrentTime, true); event.target.playVideo(); }); } else { setTimeout(checkAndCreate, 100); } }; checkAndCreate(); } return () => { if (fullscreenPlayerRef.current?.destroy) { fullscreenPlayerRef.current.destroy(); fullscreenPlayerRef.current = null; } }; }, [isFullscreen, playerReady, selectedProject, videoCurrentTime]); // Removed currentMediaIndex here as it's always 0 for video fullscreen entry
...
const renderMedia = (project: Project, index: number, inFullscreen: boolean = false) => {
    ...
    const commonClasses = inFullscreen ? "object-contain rounded-lg" : "object-cover rounded-lg"; // Removed max-w-full max-h-full for non-fullscreen as fill handles it
    ...
    if (isVideo && project.videoUrl) {
      ...
      const iframeKey = `${baseId}-${videoId}`; // Ensure key is unique and stable
      ...
      return (
        <div className={`relative ${inFullscreen ? 'w-full h-full flex items-center justify-center' : 'w-full h-56'} ${!inFullscreen ? 'cursor-pointer' : ''}`} onClick={!inFullscreen ? handleMediaClick : undefined} role={!inFullscreen ? 'button' : undefined} aria-label={!inFullscreen ? 'View image fullscreen' : undefined} >
          <Image
            ...
            fill={!inFullscreen}
            width={inFullscreen ? 1200 : undefined} // Only needed if fill is false
            height={inFullscreen ? 900 : undefined} // Only needed if fill is false
            ...
            quality={inFullscreen ? 90 : 75}
            // **** REFINED PRIORITY PROP ****
            // Prioritize the first *image* shown in the details panel.
            // For fullscreen, it's already visible, so prioritize that too.
            priority={
              inFullscreen || // If fullscreen, it's the main content
              (!inFullscreen && // Only for the details panel view
                ( (project.videoUrl && index === 1) || (!project.videoUrl && index === 0) ) // True if it's the first *image*
              )
            }
            unoptimized={false}
          />
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen p-2 pt-8">
      <div className="w-full max-w-7xl px-4 flex flex-col items-center">
        ...
        <div className="flex justify-between w-full relative gap-8" /* Removed ref={projectsContainerRef} as grid is now directly styled */ >
          <div
            ref={projectsContainerRef} // Moved ref here
            className={`grid gap-8 ${ selectedProject && !isMobile ? "w-[calc(70%-1rem)]" : "w-full" }`}
            // style={{ gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))" }} // This will be set by useEffect
          >
             {sortedProjects.map((project) => ( <div key={project.id} className={`bg-gray-800 rounded-lg overflow-hidden shadow-lg cursor-pointer select-none hover:scale-105 transition-transform ${ selectedProject?.id === project.id ? "ring-2 ring-blue-500 scale-105" : "" }`} onClick={() => handleProjectClick(project)} onDragStart={(e) => e.preventDefault()} style={{ maxHeight: "400px" }} >
               <div className="relative w-full h-48">
                 <Image
                    ...
                    quality={80} // Slightly reduced for thumbnails if many
                    priority={project.relevance > 5} // Keep prioritizing important thumbnails
                    // loading="eager" // 'eager' can be okay for above-the-fold thumbnails, 'lazy' for below
                 />
                </div>
                <div className={`p-3 ${selectedProject?.id === project.id ? "bg-blue-900" : "bg-gray-800"}`}>
                  <h3 className="text-lg font-semibold text-white truncate" title={project.title}> {project.title} </h3>
                  <div className="flex flex-wrap mt-2 mb-1"> {project.tags.slice(0, 3).map((tagKey) => ( <span key={tagKey} className={`${TAGS[tagKey].color} text-white text-xs px-2 py-1 rounded mr-2 mb-1`} > {TAGS[tagKey].name} </span> ))} {project.tags.length > 3 && ( <span className="text-gray-400 text-xs py-1 mb-1">...</span> )} </div>
                  <div className="text-sm text-gray-400 mt-1"> Created: {project.createdAt.toLocaleDateString()} </div>
                </div>
              </div> ))}
          </div>
          ...
          {selectedProject && (
            ...
            >
              <button
                ...
                className="absolute top-2 right-2 text-white bg-red-500 hover:bg-red-700 w-6 h-6 flex items-center justify-center text-sm font-bold z-50 rounded-sm" /* Adjusted rounding */
              ...
              <div className="p-4">
                <div className="relative w-full h-56 mb-4 bg-black rounded-lg overflow-hidden">
                  {renderMedia(selectedProject, currentMediaIndex, false)}
                   ...
                   {/* Fullscreen button for video is handled inside renderMedia */}
                </div>
