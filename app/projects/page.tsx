...
async function fetchRepoCreationDate(repoName: string): Promise<Date | null> {
  try {
    ...
    const response = await fetch(`${GITHUB_API_BASE}/repos/${GITHUB_USERNAME}/${repoName}`, { headers });

    if (!response.ok) {
      console.error(`Failed to fetch repo data for ${repoName}: ${response.status} ${response.statusText}`);
      return null;
    }
...
function Projects() {
  ...
  useEffect(() => {
    ...
    if (detailsPlayerRef.current?.destroy) {
      try { detailsPlayerRef.current.destroy(); } catch (e) { console.error("Error destroying details player on project change:", e); }
      detailsPlayerRef.current = null;
    }
    if (fullscreenPlayerRef.current?.destroy) {
      try { fullscreenPlayerRef.current.destroy(); } catch (e) { console.error("Error destroying fullscreen player on project change:", e); }
      fullscreenPlayerRef.current = null;
    }

    if (playerReady && selectedProject && selectedProject.videoUrl) {
      createPlayer(`youtube-iframe-details-${selectedProject.id}`, detailsPlayerRef);
    }
  }, [selectedProject, playerReady]);
 ...
 useEffect(() => { if (!(window as any).YT || !(window as any).YT.Player) { const tag = document.createElement('script'); tag.src = 'https://www.youtube.com/iframe_api'; document.body.appendChild(tag); (window as any).onYouTubeIframeAPIReady = () => { setPlayerReady(true); }; } else { setPlayerReady(true); } return () => { if (typeof (window as any).onYouTubeIframeAPIReady === 'function') { delete (window as any).onYouTubeIframeAPIReady; }}; }, []);
  ...
  const createPlayer = (elementId: string, playerRef: React.MutableRefObject<any>, onReadyCallback?: (event: any) => void) => { if (!playerReady || !document.getElementById(elementId)) return; if (playerRef.current?.destroy) { try { playerRef.current.destroy(); } catch (e) { console.error("Error destroying player in createPlayer:", e); } playerRef.current = null; } playerRef.current = new (window as any).YT.Player(elementId, { events: { 'onReady': (event: any) => { if (onReadyCallback) onReadyCallback(event); }, 'onError': (event: any) => { console.error(`YT Error ${elementId}:`, event.data); }, } }); };

  useEffect(() => { if (playerReady && selectedProject && selectedProject.videoUrl && currentMediaIndex === 0 && !isFullscreen) { const detailsElementId = `youtube-iframe-details-${selectedProject.id}`; if (document.getElementById(detailsElementId) && (!detailsPlayerRef.current || detailsPlayerRef.current.getIframe()?.id !== detailsElementId)) { createPlayer(detailsElementId, detailsPlayerRef); } } return () => { if (detailsPlayerRef.current && (!selectedProject || currentMediaIndex !== 0)) { if (detailsPlayerRef.current?.destroy) { try { detailsPlayerRef.current.destroy(); } catch(e) { console.error("Error destroying details player in cleanup:", e); }} detailsPlayerRef.current = null; } }; }, [playerReady, selectedProject, currentMediaIndex, isFullscreen]);

  useEffect(() => {
    if (isFullscreen && playerReady && selectedProject && selectedProject.videoUrl && currentMediaIndex === 0) {
      const fullscreenElementId = `youtube-iframe-fullscreen-${selectedProject.id}`;
      const checkAndCreate = () => {
        if (document.getElementById(fullscreenElementId)) {
          createPlayer(fullscreenElementId, fullscreenPlayerRef, (event) => {
    ...
    return () => {
      if (fullscreenPlayerRef.current?.destroy) {
        try { fullscreenPlayerRef.current.destroy(); } catch (e) { console.error("Error destroying fullscreen player in cleanup:", e); }
        fullscreenPlayerRef.current = null;
      }
    };
  }, [isFullscreen, playerReady, selectedProject, videoCurrentTime, currentMediaIndex]); 
...
const renderMedia = (project: Project, index: number, inFullscreen: boolean = false) => {
    ...
    const commonClasses = inFullscreen ? "object-contain rounded-lg" : "object-cover rounded-lg"; 
    ...
    if (isVideo && project.videoUrl) {
      ...
      const shouldPrioritize: boolean = !!(inFullscreen || (index === 0 && !project.videoUrl) || (index > 0 && project.videoUrl));
      return (
        <div className={`relative w-full h-full ${!inFullscreen ? 'cursor-pointer' : ''}`} onClick={!inFullscreen ? handleMediaClick : undefined} role={!inFullscreen ? 'button' : undefined} aria-label={!inFullscreen ? 'View image fullscreen' : undefined} >
          <Image
            src={imageUrl}
            ...
            priority={shouldPrioritize} // This should now be fine
            unoptimized={false}
          />
        </div>
      );
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen p-2 pt-8">
      ...
      <div className="w-full max-w-7xl px-4 flex flex-col items-center">
        ...
        <div className="flex justify-between w-full relative gap-8" ref={projectsContainerRef} >
          ...
          {selectedProject && (
            ...
            >
              <button onClick={closeProjectDetails} className="absolute top-2 right-2 text-white bg-red-500 hover:bg-red-700 w-6 h-6 flex items-center justify-center text-sm font-bold z-50 " aria-label="Close project details" > × </button>
              <div className="p-4">
                  ...
                  {selectedProject.videoUrl && (() => {
                    ...
                    return (
                      <div
                        ...
                        className={`inline-block align-top w-28 h-20 mr-1 cursor-pointer rounded-md overflow-hidden relative ${currentMediaIndex === 0 ? 'ring-2 ring-blue-500' : 'ring-1 ring-gray-700 hover:ring-gray-500'}`}
                        onClick={() => setCurrentMediaIndex(0)}
                        title={selectedProject.title + " - Video"}
                      >
                        {videoId ? ( <Image src={`https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`} alt="Video thumbnail" fill className="object-cover pointer-events-none" quality={75} sizes="112px" /> ) : ( <div className="w-full h-full bg-gray-700 flex items-center justify-center pointer-events-none"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 text-white"><path fillRule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.279 20.001c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" /></svg> </div> )}
                        <div className="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center group-hover:bg-opacity-10 transition-opacity pointer-events-none"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 text-white opacity-75"><path fillRule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.279 20.001c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" /></svg> </div>
                      </div> );
                  })()}
                  {selectedProject.images.map((imageUrl, imgIdx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`inline-block align-top w-28 h-20 mr-1 cursor-pointer rounded-md overflow-hidden relative ${currentMediaIndex === overallIndex ? 'ring-2 ring-blue-500' : 'ring-1 ring-gray-700 hover:ring-gray-500'}`}
                ...
                <h3 className="text-xl font-semibold text-white mt-1 mb-3 break-words"> {selectedProject.title} </h3>
                ...
                <p className="text-gray-400 text-sm mb-5"> Created: {selectedProject.createdAt instanceof Date ? selectedProject.createdAt.toLocaleDateString() : 'Loading date...'} </p>
      ...
      {isFullscreen && selectedProject && (
        // MODIFIED: Increased padding on the fullscreen wrapper from p-1 to p-8
        <div className="fixed inset-0 bg-black bg-opacity-95 z-[100] flex flex-col items-center justify-center p-12 animate-fade-in">
          ...
          <div className="relative w-full flex-grow flex items-center justify-center overflow-hidden p-1"> {/* p-1 here keeps a small gap for the media itself from the edges of this flex-grow container */}
          ...
          {getTotalMediaCount(selectedProject) > 1 && (
            ...
            >
              {selectedProject.videoUrl && (() => {
                const videoId = selectedProject.videoUrl.split("v=")[1]?.split("&")[0];
                return (
                  <div key={`fs-video-thumb-${selectedProject.id}`}
                    className={`inline-block flex-shrink-0 w-24 h-20 cursor-pointer rounded-md overflow-hidden relative align-top ${currentMediaIndex === 0 ? 'ring-2 ring-blue-400' : 'ring-1 ring-gray-600 hover:ring-gray-400'}`}
                    onClick={() => handleFullscreenThumbnailClick(0)}
                    title={selectedProject.title + " - Video"}
                  >
                    {videoId ? (
                      <Image src={`https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`} alt="Video thumbnail" fill className="object-cover pointer-events-none" quality={70} sizes="96px" />
                    ) : (
                      <div className="w-full h-full bg-gray-700 flex items-center justify-center pointer-events-none">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-white"><path fillRule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.279 20.001c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" /></svg>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black bg-opacity-20 flex items-center justify-center group-hover:bg-opacity-0 transition-opacity pointer-events-none">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-white opacity-60"><path fillRule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.279 20.001c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" /></svg>
                    </div>
                  </div>
                );
              })()}
              {selectedProject.images.map((imageUrl, imgIdx) => {
                const overallIndex = selectedProject.videoUrl ? imgIdx + 1 : imgIdx;
                return (
                  <div key={`fs-img-thumb-${selectedProject.id}-${imgIdx}`}
      ...
      {hoveredTag && ( <div ref={tooltipRef} className="fixed bg-gray-900 text-white text-xs p-2 rounded z-[130] break-words w-40 shadow-lg pointer-events-none" style={{ left: `${tooltipPosition.x}px`, top: `${tooltipPosition.y}px`, transform: tooltipPosition.alignTop ? 'translateY(calc(-100% - 5px))' : 'translateY(5px)', transition: 'top 0.1s ease-out, left 0.1s ease-out', }} > {TAGS[hoveredTag].description} </div> )}
    </div>
  );
}
